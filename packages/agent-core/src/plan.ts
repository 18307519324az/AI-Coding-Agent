import type { PlanOutput, ProjectContext, SelfReviewOutput, TestResult } from "@ai-coding-agent/shared";
...
export function createInitialPlan(input: {
  ...
  const targetFiles = context?.relevantFiles.length
    ? context.relevantFiles.slice(0, 12)
    : ["package.json", "src/**/*", "tests/**/*", "playwright.config.*"];
  const commands = context
    ? Object.values(context.recommendedCommands).filter((command): command is string => Boolean(command))
    : [];
  const testStep = commands.length
    ? `Run allowed verification commands: ${commands.join(", ")}.`
    : "Run lint, typecheck, unit tests, and Playwright checks when applicable.";

  return {
    summary: `Investigate "${input.title}" in a ${context?.projectKind ?? input.projectKind ?? "unknown"} project, identify the smallest safe change, add regression coverage, and prepare a reviewed PR.`,
    assumptions: [
    ...
    risks: [
      ...
      context?.hasFrontend
        ...
        : "No front-end framework was detected, so browser verification may be skipped.",
